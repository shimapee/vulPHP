...
class Dao {
	...
	function getUserDate($uid) {
		$sql = "select user_id, user_email, user_name, date_format(user_birthday,'%Y/%m/%d') as user_birthday, user_address from user where user_id = " . $uid;
	...
	function getComment($isbn13) {
		$sql = "select u.user_name, c.comment from comment c inner join user u on c.comm_user_id = u.user_id 
